...
export class AccessToken {
    ...
    public findOne(access_token: AccessTokenType): Promise<string> {
        return new Promise<string>((resolve, reject) =>
            this.redis.get(access_token, (err: Error | undefined | null, user_id: string | undefined | null) => {
    ...
    public logout(arg: LogoutArg, callback: (err?: Error | RestError) => void) {
        ...
        // TODO: Rewrite this in Lua [maybe?]
            this.redis.smembers(arg.user_id, (err: Error | null | undefined, access_tokens: string[] | undefined) => {
                if (err != null) return callback(err);
                else if (access_tokens === undefined) return callback(new TypeError("access_tokens === undefined"));
                (this['redis'] as any)
                    ['multi']()
                    ['del'](...access_tokens)
                    ['exec'](errors =>
            ...
            this.redis.get(arg.access_token, (err: Error | undefined | null, user_id: string | undefined | null) => {
                if (err != null) return callback(err);
    ...
    public add(user_id: string, roles: string, scope: 'access',
               callback: (err: Error | undefined | null, access_token: AccessTokenType) => void) {
        const new_key: string = `${roles}::${scope}::${uuid_v4()}`;
        const t = this.redis.multi();
        t.set(new_key, user_id);
        t.sadd(user_id, new_key);
        const exec_cb = (err: Error | undefined | null) => callback(err, new_key);
        t
            .exec(exec_cb)
            .catch(exec_cb);
    }
